...
function passwordProtected(req, res, next) {
  ...
  if (req.headers.authorization == "Basic RnJhbmNrOkthbnph") {
    next();
  } else {
...
app.post("/update-item", async function (req, res) {
  await db
    .collection("items")
    .findOneAndUpdate(
      { _id: new ObjectId(req.body.data) },
...
app.post("/delete-item", async function (req, res) {
  await db.collection("items").deleteOne({ _id: new ObjectId(req.body.data) });
  res.send("suprimer avec succés");
});
